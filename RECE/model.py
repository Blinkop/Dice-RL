...
class EntrywiseLinear(nn.Module):
    def __init__(self, hidden_units, n_entries):

        super(EntrywiseLinear, self).__init__()

        self.weight = nn.Parameter(torch.randn(n_entries, hidden_units, hidden_units, requires_grad=True))

    def forward(self, inputs):
        return torch.matmul(self.weight, inputs.unsqueeze(-1)).squeeze(-1)
        # return vmap(torch.matmul)(self.weight, inputs.unsqueeze(-1))
...
class SASRecBackBone(nn.Module):
    def __init__(self, item_num, config, item_emb_svd):
        ...
        if config["lin_layer_dim"] > 0:
            # self.item_emb_layer = EntrywiseLinear(config['hidden_units'], config['maxlen'])
            self.item_emb_layer = nn.Linear(config["lin_layer_dim"], config['hidden_units'], bias=False)
...
class SASRec(SASRecBackBone):
    def __init__(self, item_num, config, item_emb_svd=None):
        super().__init__(item_num, config, item_emb_svd)
