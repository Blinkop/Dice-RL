...
def sample_dross(all_items, user_items, maxlen, pad_token, n_neg_samples, random_state):
    ...
    neg = random_state.choice(
        all_items[~np.isin(all_items, user_items)], n_neg_samples, replace=False
    )
...
def sample_with_rep(
    user_items, maxlen, pad_token, n_neg_samples, itemnum, random_state
):
    seq = np.full(maxlen, pad_token, dtype=np.int32)
    pos = np.full(maxlen, pad_token, dtype=np.int32)
    ...
    seq[-n_user_items:] = user_items[-n_user_items - 1 : -1]
    pos[-n_user_items:] = user_items[-n_user_items:]
    neg[:, -n_user_items:] = random_state.randint(
        0, itemnum, (n_neg_samples, n_user_items)
    )
...
class SequentialDataset(Dataset):
    def __init__(
        ...
        self.valid_users = [
            user for user in range(usernum) if len(user_train.get(user, [])) > 1
        ]
    ...
    def __getitem__(self, idx):
        ...
        if self.sampling == "with_rep":
            ...
            seq, pos, neg = sample_without_rep(
                user_items,
                self.maxlen,
                self.pad_token,
                self.n_neg_samples,
...
def data_to_sequences_rating(data, data_description, n_neg_samples=-1):
    ...
    sequences = (
        ...
        .groupby(userid, sort=False)["itemid_rating"]
        .apply(list)
    )
    if n_neg_samples > 0:
