...
def recommend_sequential(
    ...
    *,
    ...
    '''
    model.eval()
    predictions = predict_sequential(model, target_seq[:-1], seen_seq, user=user, item_embs=item_embs)
...
def predict_sequential(model, target_seq, seen_seq, user, item_embs): # example for SASRec
...
def main():
    ...
    training_temp, data_description_temp, testset_valid_temp, testset, holdout_valid_temp, _ = get_dataset(local_file=data_path,
                                                                                     splitting='temporal_full',
                                                                                     q=0.8)
    ...
    model_D_sasrec, _, _, _ = prepare_sasrec_model(model_D_conf, training_temp, data_description_temp, device)
    model_D_sasrec.load_state_dict(torch.load("./saved_models/model_D_sasrec.pt", map_location=torch.device(device)))

    model_e0, _, _, _= prepare_sasrec_model(model_e0_conf, training_temp, data_description_temp, device)
    model_e0.load_state_dict(torch.load("./saved_models/model_e0.pt", map_location=torch.device(device)))

    model_e1, _, _, _ = prepare_sasrec_model(model_e1_conf, training_temp, data_description_temp, device)
    model_e1.load_state_dict(torch.load("./saved_models/model_e1.pt", map_location=torch.device(device)))

    model_e2, _, _, _ = prepare_sasrec_model(model_e2_conf, training_temp, data_description_temp, device)
    model_e2.load_state_dict(torch.load("./saved_models/model_e2.pt", map_location=torch.device(device)))
    ...
    for model_name, (model, item_embs_var) in models.items():
        model.eval()
        val_scores = sasrec_model_scoring(model, testset_valid_temp, data_description_temp, item_embs_var, device)
        ...
        val_metrics = model_evaluate(val_recs, holdout_valid_temp, data_description_temp)
    ...
    for model_name, (model, item_embs_var) in models.items():
        ...
        for user, test_seq in tqdm(test_sequences.items(), total=len(test_sequences)):
            ...
            # Get predicted items
            predicted_items = recommend_sequential(model, test_seq, seen_seq, topn, user=user, item_embs=item_embs_var)
            
            # compute hit steps and indices
            # print(predicted_items == np.atleast_2d(test_seq).T)
            hit_steps, hit_index = np.nonzero(predicted_items == np.atleast_2d(test_seq).T)
